...
export async function buildDashboardData(wh) {
  ...
  // --- COST ---
  // Lấy cả Tổng chi phí (G=index 6) và KG/ngày (D=index 3) để tính Cost/kg theo
  // tuần/tháng = Σ(Tổng chi phí)/Σ(KG) (có trọng số), thay vì trung bình cộng.
  const dates = [];
  const costKg = [];
  const cost = [];
  const kg = [];
  for (const r of costRows.slice(1)) {
    ...
    costKg.push(vnNum(r[7])); // Cost/kg theo ngày (cột H)
    cost.push(vnNum(r[6])); // Tổng chi phí (cột G)
    kg.push(vnNum(r[3])); // KG/ngày (cột D)
  }
  const COST_DATA = { dates, costKg, cost, kg };
  ...
  if (wh.hr && wh.hr.tab) {
    try {
      ...
      // Các dòng số lượng NVCT (cột H bắt đầu bằng "NVCT") -> tổng NVCT/ngày làm trọng số.
      const nvctRows = hrRows.filter((r) => str(r[7]).toUpperCase().startsWith('NVCT'));
      if (flRow && dateCols.length) {
        ...
        for (const { i, iso } of dateCols) {
          const v = flRow[i];
          if (v != null && str(v) !== '') {
            map[iso] = vnNum(v); // "26%" -> 26
            let w = 0;
            for (const nr of nvctRows) w += vnNum(nr[i]);
            wmap[iso] = w;
          }
        }
        const dates = Object.keys(map).sort();
        HR_DATA = { dates, fl: dates.map((d) => map[d]), nvct: dates.map((d) => wmap[d]) };
      }
    } catch {
      HR_DATA = { dates: [], fl: [] };
    }
  ...
  // Ánh xạ ngày -> số tuần thực tế (cột B "Tuần") để gom & ghi nhãn biểu đồ tuần.
  const WEEK_MAP = {};
  for (const r of ltRows.slice(1)) {
    if (hasDate(r, 2)) { const wk = smartNum(r[1]); if (wk > 0) WEEK_MAP[dmyToIso(r[2])] = wk; }
  }
  for (const r of costRows.slice(1)) {
    if (hasDate(r, 2)) { const iso = dmyToIso(r[2]); if (!(iso in WEEK_MAP)) { const wk = smartNum(r[1]); if (wk > 0) WEEK_MAP[iso] = wk; } }
  }

  return { LT_DATA, COST_DATA, PROD_DATA, NOTES_DATA, HR_DATA, WEEK_MAP };
}
