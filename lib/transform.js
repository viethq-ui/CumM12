...
export async function buildDashboardData() {
  ...
  // --- Năng suất ---
  // app cần: [week, date, slot, type, _, _, _, _, _, donH, _, wH]
  const PROD_DATA = prodRows
    ...
    .map((r) => {
      ...
      const donH = usNum(r[10]); // Năng suất (Đơn/h)
      const wH = usNum(r[12]); // Năng suất (W/h)
      return [week, dmyToIso(r[2]), slot, type, 0, 0, 0, 0, 0, donH, 0, wH];
    });
