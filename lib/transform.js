...
export async function buildDashboardData() {
  ...
  const PROD_DATA = prodRows
    ...
    .map((r) => {
      const week = smartNum(r[13]); // Week_Sort
      const slot = cleanSlot(r[3]); // Khung giờ
      const type = str(r[4]); // receive/deliver
      const labH = smartNum(r[8]); // Số giờ công đáp ứng
      const totVol = smartNum(r[9]); // Tổng sản lượng (đơn)
      const weight = smartNum(r[11]); // Khối lượng (Kg)
      const donH = smartNum(r[10]); // Năng suất (Đơn/h) theo dòng
      const wH = smartNum(r[12]); // Năng suất (W/h) theo dòng
